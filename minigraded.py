...
minigraded = Flask(__name__)    
# Put your own secret key here. You can't have mine!
minigraded.secret_key = '!\xec\xa8\x88\xc9R\xf7i<wW\x9fzH\x81\xff\x11~aQn\x9f\xcf\x0b'
...
def grade_stream(assignment, repo):
    ...
    try:
        ...
        with open(str(time.time())+".result", 'w') as results:
            ...
            #shutil.copy("/home/grader/minigrade-dave/tests/testfiles/abc.txt", "abc.txt")
            if build:
...
@minigraded.route('/auth/login', methods=['POST', 'GET'])
def login():
...
@minigraded.route('/auth/logout', methods=['POST'])
def logout():
...
if __name__ == '__main__':
    minigraded.run(host='0.0.0.0', debug=False, threaded=True, port=PORT_NUMBER)
    #minigraded.run(debug=True, threaded=True, port=9080)
